...
def guess_mount_roots():
    ...
    try:
        with open("/proc/mounts", "r", encoding="utf-8", errors="ignore") as f: #this reads from the device
            for line in f:
                parts = line.split()
                if len(parts) >= 3:
                    mnt = parts[1]
...
class ShipmentList:
    ...
    def parse(text: str):
        ...
        tokens = re.findall(r"(?<!\d)\d{10}(?!\d)", text) #this is based on stackoverflow regex for 10-digit runs
...
class MainWindow(QStackedWidget):
    def __init__(self):
        ...
        self.menu.shipSelected.connect(lambda: self.setCurrentIndex(2)) #this goes to ship mode
        self.menu.viewOrderSelected.connect(lambda: self.setCurrentIndex(3)) #this goes to view order
...
if __name__ == "__main__":
    app = QApplication(sys.argv) #this starts the qt app
    w = MainWindow() #this builds the main window
    w.show() #this shows the ui
    sys.exit(app.exec_()) #this blocks until the app closes
