...
this version builds on the previous version but adds a completion screen after the ship screen, progress bar, red lights added, and elimated double barcode references
'''
        ...
        while not self._stop:
            try:
                if self._mode == "standby":
                    ...
                    for i in range(self.strip0.led_count):
                        self.strip0.RGBto3Bytes(i, 255, 0, 0)
                        self.strip1.RGBto3Bytes(i, 255, 0, 0)
                    self.strip0.LED_show()
                    self.strip1.LED_show()
                    self.msleep(150)
...
class DualPingWorker(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            self.log.emit("alternating MB1040 readings every 3s...")
            while not self._stop:
            ...
            while not self._stop:
                ...
                if decoded:
                    ...
                    if expected_total and len(self._found) >= expected_total:
                        self.log.emit("all items are loaded :)")
...
class ShipScreen(QWidget):
    def __init__(self):
        ...
        title = QLabel("Shipment in progress..")
...
class MainWindow(QStackedWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("palletPortalv26")
