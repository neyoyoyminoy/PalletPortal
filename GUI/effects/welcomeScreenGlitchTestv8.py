...
class GlitchText(QWidget):
    ...
    def paintEvent(self, e):
        ...
        # build a painter path of the TEXT ITSELF
        path = QPainterPath()
        path.addText(0, 0, self.font, self.scrambled)
        bounds = path.boundingRect()  # real geometric bounds of rendered text

        # center the path
        x = (self.width() - bounds.width()) / 2 - bounds.left()
        y = (self.height() - bounds.height()) / 2 - bounds.top()

        # BASE WHITE
        p.setPen(QColor(255, 255, 255))
        p.drawText(x, y + bounds.height(), self.scrambled)
        ...
        if self.glitch_strength > 0:
            ...
            p.drawText(x - shift, y + bounds.height(), self.scrambled)
            ...
            if random.random() < 0.4:
                ...
                jitter_y = y + bounds.height() + random.randint(-20, 20)
