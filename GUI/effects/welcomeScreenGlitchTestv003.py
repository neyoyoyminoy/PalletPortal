...
and adds the exit keys to escape the program since we are making this fullscreen now

the welcome text should be centered now
"""
...
#based on guiv21/guiv27 spitoWS class
class SPItoWS:
...
class GlitchText(QWidget):
    ...
    def paintEvent(self, e):
        ...
        text_h = fm.height() #text height

        x = (self.width() - text_w) // 2 #sets the font og position
    ...
    def keyPressEvent(self, e): #exit sequence
        self._pressed = getattr(self, "_pressed", set())  #init if missing
        self._pressed.add(e.key())

        if {Qt.Key_Control, Qt.Key_C, Qt.Key_V, Qt.Key_Return}.issubset(self._pressed): #all four buttons
            QApplication.quit()

    def keyReleaseEvent(self, e):
        self._pressed.remove(e.key()) #removing key release
