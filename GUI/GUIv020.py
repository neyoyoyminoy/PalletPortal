'''
guiv20_corrected.py
this version embeds the spi-based ws2812 driver for jetson orin nano.
uses pin 19 (spi0_mosi) for led data.
enable spi0 via: sudo /opt/nvidia/jetson-io/jetson-io.py
...
# -------------------- ws2812 strip (spi) --------------------
#based on https://github.com/seitomatsubara/Jetson-nano-WS2812-LED-/blob/master/W2812.py
#note: enable spi0 via jetson-io; pin19=spi0_mosi. use 330Ω resistor + 1000µf cap.
class WS2812Strip:
    def __init__(self,count=5,spi_bus=0,spi_dev=0,brightness=128):
        self.count=count; self.brightness=max(0,min(255,int(brightness))); self.ok=False
        try:
            import spidev,numpy as np
            self._np=np; self.spi=spidev.SpiDev(); self.spi.open(spi_bus,spi_dev)
            self.spi.max_speed_hz=6400000; self.ok=True
        except Exception as e:
            self.spi=None; print(f"[warn] spi ws2812 init failed: {e}")
    def _encode(self,data):
        enc=[]
        for val in data:
            v=int(val)&0xff
            for bit in range(7,-1,-1):
                enc.append(0b11100000 if (v>>bit)&1 else 0b10000000)
        return self._np.array(enc,dtype=self._np.uint8)
    def _apply_brightness(self,r,g,b):
        br=self.brightness/255.0; return int(r*br),int(g*br),int(b*br)
    def set_all(self,r,g,b,show=True):
        if not self.ok:return
        r,g,b=self._apply_brightness(r,g,b); raw=[]
        for _ in range(self.count): raw+=[g&0xff,r&0xff,b&0xff]
        try:self.spi.xfer2(self._encode(raw).tolist())
        except:pass
    def set_pixels(self,colors,show=True):
        ...
        for i in range(self.count):
            r,g,b=(0,0,0)
            if i<len(colors): r,g,b=colors[i]
            r,g,b=self._apply_brightness(r,g,b); raw+=[g&0xff,r&0xff,b&0xff]
        try:self.spi.xfer2(self._encode(raw).tolist())
        except:pass
    def clear(self): self.set_all(0,0,0)

# -------------------- led worker --------------------
class LEDWorker(QThread):
    to_standby=pyqtSignal(); to_green=pyqtSignal(); to_yellow_pulse=pyqtSignal()
    def __init__(self,num_leds=5,brightness=128,parent=None):
        super().__init__(parent)
        self.strip=WS2812Strip(count=num_leds)
        self._mode="standby"; self._stop=False
        ...
        while not self._stop:
            try:
                if self._mode=="standby":
                    if self.strip.ok:
                        colors=[self._wheel((idx+i*10)%256) for i in range(self.strip.count)]
                        self.strip.set_pixels(colors)
                    self.msleep(50)
                elif self._mode=="green":
                    if self.strip.ok:self.strip.set_all(0,150,0)
                    self.msleep(150)
                elif self._mode=="yellow_pulse":
                    if self.strip.ok:
                        self.strip.set_all(255,160,0); self.msleep(120)
                        self.strip.set_all(0,0,0); self.msleep(120)
                        self.strip.set_all(255,160,0); self.msleep(120)
                        self.strip.set_all(0,0,0); self.msleep(600)
                    else:self.msleep(200)
                else:self.msleep(100)
                idx=(idx+5)%256
            except: self.msleep(200)
        try:
            if self.strip.ok:self.strip.clear()
        except: pass
