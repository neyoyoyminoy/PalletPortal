...
class ShipScreen(QWidget):
    def __init__(self):
        ...
        self._barcode_items = {}  # maps barcode -> QListWidgetItem for marking scanned

        # --- internal state ---
        self._expected_codes = []     # manifest codes from USB
        self._found = set()           # codes already scanned
        self._barcode_worker = None   # first camera worker
        self._barcode_worker2 = None  # second camera worker
        self.worker = None            # ping worker
    ...
    def _start_usb_removal_watcher(self):
        # Timer to check every second
        self.usb_timer = QTimer(self)
        self.usb_timer.timeout.connect(self._check_usb_removed)
        self.usb_timer.start(1000)

    def _check_usb_removed(self):
        usb_path = "/media/usb"  # replace with your USB mount point
        if not os.path.ismount(usb_path):
            self.usb_timer.stop()
            self._log("USB removed, returning to Welcome screen...")
            self._return_to_welcome()

    def _return_to_welcome(self):
        # Reset Ship screen if needed
        self._expected_codes = []
        self._found.clear()
        self.scanned_list.clear()
        self.status.setText("Waiting for ping sensor...")

        # Switch back to Welcome screen
        parent = self.parent()
        if parent is not None:
            parent.setCurrentIndex(0)  # assuming Welcome screen is index 0
    ...
    def _on_ready(self, dist_in: float, label: str):
        ...
        # --- start both barcode workers ---
        for idx in range(2):
            worker_attr = f"_barcode_worker{'' if idx == 0 else 2}"
            if getattr(self, worker_attr) is None:
                ...
                worker.matched.connect(lambda val, sc=None, m=None: self._on_match(val))
                worker.decoded.connect(self._on_match)  # triggers LEDs on first decode
                worker.finished_all.connect(self._on_all_done)
                setattr(self, worker_attr, worker)
                worker.start()

        # --- first decode LED pulse ---
        def _first_decode(_val):
            if self._leds:
                self._leds.to_yellow_pulse.emit()
            # disconnect from both workers
    ...
    # Update status
        self.status.setText("All barcodes scanned — shipment complete\nPlease remove flash drive")
     ...
     # update LEDs to indicate shipment complete
        if self._leds:
            self._leds.to_green.emit()  # solid green for completion

    # optionally mark all items in scanned_list as completed visually
        for index in range(self.scanned_list.count()):
            item = self.scanned_list.item(index)
            item.setForeground(Qt.green)
            item.setFont(QFont("Beausite Classic", 12, QFont.Bold))

        # start USB removal watcher
        self._start_usb_removal_watcher()
