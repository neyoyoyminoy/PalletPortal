'''
building on version 22
this fixes the integration of the  the second LED strip to run at the same time
spi-based ws2812 integration for 5 leds on pin 19 (spi0_mosi) and 5 more leds on pin 37 (spi1_mosi)
...
import spidev #for spi pins for led strips
...
#-------------------- ws2812 led strip (spi0 using pin 19) --------------------
#based on https://github.com/seitomatsubara/Jetson-nano-WS2812-LED-/blob/master/W2812.py
#note: enable spi0 via jetson-io; pin19=spi0_mosi. use 330Ω resistor + 1000µf cap.
class SPItoWS:
    def __init__(self, ledc=5, spi_bus=0, spi_dev=0):
        self.led_count = ledc
...
# --- led worker controlling both strips ---
class LEDWorker(QThread):
    ...
    def __init__(self, num_leds=5, parent=None):
        super().__init__(parent)
        #main strip on spi0.0 (pin19) and second strip on spi1.0 (pin37)
        self.strip1 = SPItoWS(num_leds, spi_bus=0, spi_dev=0)
        self.strip2 = SPItoWS(num_leds, spi_bus=1, spi_dev=0)
        self._mode = "standby"
        self._stop = False
    ...
    def _apply_to_both(self, func):
        #helper to mirror updates to both strips
        try: func(self.strip1)
        except Exception: pass
        try: func(self.strip2)
        except Exception: pass
    ...
    def _rainbow_step(self, idx):
        def update(strip):
            for i in range(strip.led_count):
                ...
                strip.RGBto3Bytes(i, r, g, b)
            strip.LED_show()
        self._apply_to_both(update)
            ...
            try:
                if self._mode == "standby":
                    ...
                    def pulse(strip):
                        for _ in range(2):
                            for i in range(strip.led_count):
                                strip.RGBto3Bytes(i, 255, 160, 0)
                            strip.LED_show()
                            time.sleep(0.12)
                            strip.LED_OFF_ALL()
                            time.sleep(0.12)
                        time.sleep(0.6)
                    self._apply_to_both(pulse)
                else:
                    self.msleep(100)
                idx = (idx + 5) % 360
            except Exception:
