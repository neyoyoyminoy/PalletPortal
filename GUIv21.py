...
class SPItoWS:
    def __init__(self, ledc=5):
        self.led_count = ledc
        self.X = "100" * (self.led_count * 8 * 3)
        self.spi = spidev.SpiDev()
        self.spi.open(0, 0)                # spi0.0 -> pin19 (spi0_mosi)
    ...
    def _Bytesto3Bytes(self, num, RGB):
        base = num * 24  # 24 bits per color triplet
        for i in range(8):
...
class LEDWorker(QThread):
    ...
    def _rainbow_step(self, idx):
        for i in range(self.strip.led_count):
            ...
            self.strip.RGBto3Bytes(i, r, g, b)
        self.strip.LED_show()
        ...
        while not self._stop:
            try:
                if self._mode == "standby":
                    ...
                    for i in range(self.strip.led_count):
                        self.strip.RGBto3Bytes(i, 0, 150, 0)
                    self.strip.LED_show()
                    self.msleep(150)
                elif self._mode == "yellow_pulse":
                    for _ in range(2):
                        for i in range(self.strip.led_count):
                            self.strip.RGBto3Bytes(i, 255, 160, 0)
                        self.strip.LED_show()
                        self.msleep(120)
                        self.strip.LED_OFF_ALL()
                        self.msleep(120)
                    self.msleep(600)
                else:
                    self.msleep(100)
